  ...
  return (
      <nav
        ...
        onMouseLeave={() => setViewMenu(false)}
          className={`${viewMenu ? "w-[325px]" : "w-fit"} px-8 py-6 min-h-screen border-r relative transition-all duration-1000`}>
          
        <div className='flex items-center py-2 mb-4'>
            <Image src={logo} alt='logo' />
              {viewMenu && <h1 className='uppercase'>Dream Affairs</h1>}  
        </div>
          <div className='py-3 px-2 space-y-4'>
              {sidebarMenu.map((item) => (
                  <Link
                      href={`${item.route}`}
                      key={item.id}
                      className={`flex items-center gap-x-3 p-3 rounded text-base leading-6 font-semibold ${params === item.route ? "text-[#101928] bg-[#E0B0FF]" : "text-[#3A3A3A] bg-none hover:text-[#101928] hover:bg-[#E0B0FF]"}`}>
                      <Image src={params === item.route ? item.iconActive : item.iconInactive} alt='icon' width={24}  height={24} className='object-center'/>
                      {viewMenu && <p className='mt-1'>{item.name}</p>}
              </Link>
              ))}
              <hr />
          </div>
        ...
        <div className='space-y-4'>
              <div className='flex items-center p-5 text-sm font-normal'>
              ...
              <div className='flex items-center'>
                <Link href={`/dashboard/settings`} className='relative'>
                  ...
                  {viewMenu && <div className='mt-1 ml-3'>
                      <p>Jane Doe</p>

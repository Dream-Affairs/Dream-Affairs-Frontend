                ...
                <div className="mt-6">
                    <form action="" method="post" onSubmit={processSendMail}>
                        ...
                        <div className="flex flex-col mb-4 mt-4">
                            ...
                            <textarea className="border rounded-md resize-none border-input px-3 py-2 focus:border-muted transition-colors duration-200 ease-in-out outline-none hover:border-primary" rows={5} name="body" defaultValue={""} id="body_id" required />
                            <span className='text-[#6F6F6F]'>Separate up to 10 recipients with commas</span>
                        </div>
                        <div className="w-full flex flex-row justify-end gap-4">
                            <Button variant={'outline'} onClick={() => modalManager(false)} className='w-1/4 h-[45px]'> Cancel</Button>
                            <Button variant={"secondary"} type="submit" className='w-1/4 h-[45px]' >Send Email</Button>
                        </div>
                    </form>
                </div>
            </div>

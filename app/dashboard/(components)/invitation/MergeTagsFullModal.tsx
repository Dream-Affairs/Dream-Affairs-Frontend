  ...
  return (
    <div>
      <Modal modal={true} hidden={hidden} setModal={modalManager}>
        <div
          className={`flex flex-col w-[650px] h-auto bg-white fixed left-[50%] top-[50%] z-50 grid  max-h-[650px] rounded-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background shadow-lg duration-200 overflow-auto transition-all ${hidden}`}
        >
          <h1 className="text-2xl font-semibold py-4">Merge tags</h1>
          <Image
            role="button"
            onClick={() => modalManager(false)}

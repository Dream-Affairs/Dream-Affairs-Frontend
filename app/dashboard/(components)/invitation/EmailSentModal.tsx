  ...
  return (
    <div>
      <Modal modal={true} hidden={hidden} setModal={modalManager}>
        <div
          className={`flex flex-col items-center w-[400px] h-auto bg-white fixed left-[50%] top-[50%] z-50 grid  max-h-[850px] rounded-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background shadow-lg duration-200 overflow-auto transition-all ${hidden}`}
        >
          <div className="flex flex-col items-center gap-4">
            ...
            <Button className="w-full" variant={'secondary'} onClick={() => modalManager(false)}>
              Done
            </Button>
          </div>
        </div>
